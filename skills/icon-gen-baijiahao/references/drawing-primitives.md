...
```
Base grid:      48 × 48 px  (official Baijiahao PC/team master)
Target canvas:  24 × 24 px  (logical model — keyline dims halved for reasoning/preview)
...
Default Baijiahao PC handoff should use the 48px base grid. Use 24px output only when the user explicitly asks for compact/export output.

---

## Stroke Scale by Canvas Size

Production-safe Baijiahao PC output is defined for the 48px team master.
...
All normal monochrome strokes: official inner stroke where supported for 48px closed shapes; center stroke only with compensated bounds. Round cap, round join, #242529.
...
- Be specific: prefer BjhBasicCommentReply over BjhBasicComment if context is reply

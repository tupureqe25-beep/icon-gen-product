...
1. Load `figma-use`.
2. Call `use_figma` with `skillNames: "figma-use,icon-gen-baijiahao"`.
...
## Output scale profiles

Baijiahao uses a 24px logical icon model, but the audited `百家号PC Component / icon / 基础元素` library stores mature icons as 48×48 masters.

Use one of two explicit output profiles:

```txt
logical-24:
  container: 24 × 24
  stroke:    2px
  use when:  user explicitly asks for compact/export 24px output outside the team library

team-master-48:
  container: 48 × 48
  scale:     multiply all 24px logical coordinates, sizes, radii, and stroke widths by 2
  stroke:    4px for standard outline glyphs; inner stroke where supported for closed shapes, compensated center stroke otherwise
  use when:  writing into Baijiahao team component-library context or matching audited mature icons
```

Do not draw unscaled 24px geometry inside a 48×48 component. It will look tiny and fail the mature-library match.
Do not include hidden `.基准线` / `图标/ 规则-24` reference layers as visible glyph geometry.
...
```js
const ICON_COLOR = { r: 36/255, g: 37/255, b: 41/255 }; // #242529
const STROKE = {
  type: 'SOLID',
  color: ICON_COLOR
};
...
function applyOfficialClosedShapeStroke(node, weight = 4) {
  node.fills = [];
  node.strokes = [STROKE];
  node.strokeWeight = weight;
...
function applyLocalTinyFill(node, reason) {
  ...
  if ('setSharedPluginData' in node) {
    node.setSharedPluginData('icon-gen-baijiahao', 'fillExceptionReason', reason || 'local tiny fill for clarity');
  }
}
```

Use `applyOfficialClosedShapeStroke` for 48px team-master closed rectangles/ellipses when possible. Use `applyIconStroke` for open paths/vectors or compensated center-stroke fallbacks.
...
```js
// Always called through use_figma with skillNames: "figma-use,icon-gen-baijiahao".
...
```jsonc
{
  "meta": { "name": "BjhBasicDownload", "size": 24 },
...
When using a Vector fallback for a line segment, do not write absolute canvas
coordinates directly inside `vectorPaths.data` while also leaving the vector at
`x=0, y=0`. Figma normalizes vector bounds, which can shift the segment visually.
Use local path coordinates and place the vector with explicit `x/y`:
...
node.vectorPaths = [{
  windingRule: 'NONZERO',
  data: `M ${shape.x1 - minX} ${shape.y1 - minY} L ${shape.x2 - minX} ${shape.y2 - minY}`
}];
```
...
```
BjhBasic{Name}  // component preferred; frame fallback allowed
└── BjhBasic{Name}__glyph  // group
...
```md
✅ 已将 `{meta.name}` 绘制为 Figma 原生可编辑节点。
- 交付形式：{component | frame fallback}
- 画布：{size}×{size}，clip content = true
- 结构：原生节点分组，未扁平化
- 图层：{count} 个，类型 {node_types}
- 描边：{weight}px {支持时内描边 | 补偿后的居中描边}，圆头/圆角，#242529
- 视觉居中修正：{shift}
- 降级/替代方案：{无 | 列表}
- 警告：{无 | 列表}
```

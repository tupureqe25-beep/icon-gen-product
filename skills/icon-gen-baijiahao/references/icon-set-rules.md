...
- Same color: #242529 for normal monochrome glyphs; canonical status-color exceptions must be shared intentionally across the set.
- Same default outline behavior: outline-first; local tiny fill only when justified by the fill exception rule.
- Same naming convention: `BjhBasic{SemanticName}`.
...
```
✅ Set: {n} icons
✅ Shared style: 24px logical / 48px team master, 2px logical / 4px team-master stroke, round cap/join, #242529
✅ Set profile: {density}, bbox {range}, primitive count {range}
✅ Naming: BjhBasic{SemanticName}
